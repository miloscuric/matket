app.controller('artikalController', [
		'$scope',
		'$location',
		'$mdDialog',
		'companyService',
		...
		function($scope, $location, $mdDialog, companyService, $timeout,
				businessPartnerService, artikalService, jediniceMereService,
				authenticationService) {
			...
			$scope.kreirajArtikal = function() {
				...
				if ("undefined" === typeof idToSend) {
					alert("Grupa artikala mora biti cekirana!");
				}

				if (!("undefined" === typeof $scope.artikaal)) {
					$scope.artikal.id = $scope.artikaal.id;
					artikalService.updateArtikal($cope.artikal).then(
							function(response) {
					...
					artikalService.createArtikal($scope.artikal, idToSend,
							idJedinice).then(function(response) {
						$location.path('/grupeArtikala');
					});
				}
			}
